...
namespace Quanyi.B2c.Yaofangwang
{
    public partial class FormMain : Form
    {
        ...
        private List<MedicineInfo> failStockUpdateList;
        private List<MedicineInfo> failPeriodUpdateList;
        //private HttpResponseModal<ApiStockInfo> failSapStockInfoList;
        //private HttpResponseModal<ApiStockInfo> failSapPeriodInfoList;

        public FormMain()
        {
            InitializeComponent();
            failStockUpdateList = new List<MedicineInfo>();
            failPeriodUpdateList = new List<MedicineInfo>();
            //failSapStockInfoList = new HttpResponseModal<ApiStockInfo>();
            //failSapPeriodInfoList = new HttpResponseModal<ApiStockInfo>();
        ...
        protected override void OnShown(EventArgs e)
        {
            ...
            //backgroundWorker1定时获取订单还是要的，只是逻辑稍微变化
        ...
        private void backgroundWorker1_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (!e.Cancelled)
                tslMsg.Text = "定时拉单程序处理完毕!";
            else
                tslMsg.Text = "定时拉单程序处理终止!";
        }
        ...
        private void backgroundWorker2_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        //MedicineStockBatchUpdate(medicineList, storage, backgroundWorker2);
                        //每次只同步20条
                        int cycle = 50;
                        //向下取整，总共循环多少次
                        ...
                        for (int i = 0; i < multiple; i++)
                        {
                            ...
                            MedicineStockBatchUpdate(currList, storage, backgroundWorker2);
                            Thread.Sleep(5 * 1000);
                        }
                        if (reminder != 0)
                        {
        ...
        private void backgroundWorker4_DoWork(object sender, DoWorkEventArgs e)
        {
            while(true)
            {
                ...
                try
                {
                    ...
                    if (medicineList != null && medicineList.Count > 0)
                    {
                        //MedicinePeriodBatchUpdate(medicineList, storage, backgroundWorker4);
                        ...
                        {
                            var currList = medicineList.Skip(i * cycle).Take(cycle).ToList();
                            MedicinePeriodBatchUpdate(currList, storage, backgroundWorker4);
                            Thread.Sleep(5 * 1000);
                        }
                        if (reminder != 0)
                        {
        ...
        private void backgroundWorker5_DoWork(object sender, DoWorkEventArgs e)
        {
            while(true)
            {
                if (failStockUpdateList.Count == 0)
                {
                    continue;
                }
                //if (failSapStockInfoList.Items == null || failSapStockInfoList.Items.Count == 0)
                //{
                //    continue;
                //}
                //MedicineStockBatchUpdate(failStockUpdateList, storage, backgroundWorker5);
                //每次只同步20条
                int cycle = 50;
                //向下取整，总共循环多少次
                int multiple = failStockUpdateList.Count / cycle;
                int reminder = failStockUpdateList.Count % cycle;
                //string storage = ConfigurationManager.AppSettings["Storage"];
                for (int i = 0; i < multiple; i++)
                {
                    var currList = failStockUpdateList.Skip(i * cycle).Take(cycle).ToList();
                    MedicineStockBatchUpdate(currList, storage, backgroundWorker5);
                    Thread.Sleep(5 * 1000);
                }
                if (reminder != 0)
                {
                    var currList = failStockUpdateList.Skip(multiple * cycle).Take(reminder).ToList();
                    MedicineStockBatchUpdate(currList, storage, backgroundWorker5);
                    Thread.Sleep(5 * 1000);
                }
            }
        ...
        private void backgroundWorker6_DoWork(object sender, DoWorkEventArgs e)
        {
            while (true)
            {
                if (failPeriodUpdateList.Count == 0)
                {
                    continue;
                }
                //if (failSapPeriodInfoList.Items == null || failSapPeriodInfoList.Items.Count == 0)
                //{
                //    continue;
                //}
                //MedicinePeriodBatchUpdate(failPeriodUpdateList, storage, backgroundWorker6);
                int cycle = 50;
                int multiple = failPeriodUpdateList.Count / cycle;
                int reminder = failPeriodUpdateList.Count % cycle;
                //string storage = ConfigurationManager.AppSettings["Storage"];
                for (int i = 0; i < multiple; i++)
                {
                    var currList = failPeriodUpdateList.Skip(i * cycle).Take(cycle).ToList();
                    MedicinePeriodBatchUpdate(currList, storage, backgroundWorker6);
                    Thread.Sleep(5 * 1000);
                }
                if (reminder != 0)
                {
                    var currList = failPeriodUpdateList.Skip(multiple * cycle).Take(reminder).ToList();
                    MedicinePeriodBatchUpdate(currList, storage, backgroundWorker6);
                    Thread.Sleep(5 * 1000);
                }
            }
        ...
        private void MedicinePeriodBatchUpdate(List<MedicineInfo> currList, string storage, BackgroundWorker bgw)
        {
            ...
            if (sapStockList.Code.Equals("S") && sapStockList.Items != null && sapStockList.Items.Count > 0)
            {
                ...
                if (result)
                {
                    ...
                    {
                        failPeriodUpdateList.Remove(item);
                    }
                ...
                else
                {
                    if(msg.Contains("频率"))
                    {
                        ...
                        foreach (var item in currList)
                        {
                            if(!failPeriodUpdateList.Exists(n=>n.PrimaryKey.Equals(item.PrimaryKey)))
                            {
                                failPeriodUpdateList.Add(item);
                            }
                        }
                        //failSapPeriodInfoList.Code = sapStockList.Code;
                        //failSapPeriodInfoList.Message = sapStockList.Message;
                        //failSapPeriodInfoList.Items = sapStockList.Items;
                    }
                    bgw.ReportProgress(PROGRESS_ERROR, $"SAP效期等信息同步失败：{msg}");
                }
            }
        }

        private void MedicineStockBatchUpdate(List<MedicineInfo> currList, string storage, BackgroundWorker bgw)
        {
            DateTime beginTime = DateTime.Now;
            var sapStockList = storage.Equals("JY02") ? _medicineService.GetPosAvailStockInfo(currList) : _sapWebApi.GetAvailableStockInfo(currList);
            DateTime endTime = DateTime.Now;
            ...
            if (sapStockList.Code.Equals("S") && sapStockList.Items != null && sapStockList.Items.Count > 0)
            {
                ...
                if (result)
                {
                    ...
                    foreach (var item in currList)
                    {
                        failStockUpdateList.Remove(item);
                    }
                }
                else
                {
                    ...
                    if(msg.Contains("频率"))
                    {
                        foreach (var item in currList)
                        {
                            if (!failStockUpdateList.Exists(n => n.PrimaryKey.Equals(item.PrimaryKey)))
                            {
                                failStockUpdateList.Add(item);
                            }
                        }
                        //failSapStockInfoList.Code = sapStockList.Code;
                        //failSapStockInfoList.Message = sapStockList.Message;
                        //failSapStockInfoList.Items = sapStockList.Items;
                    }
                    bgw.ReportProgress(PROGRESS_ERROR, $"库存同步失败：{msg}");
                }
            }
        }
        ...
        private void toolStrip1_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            {
                var frmImportOrder = new FrmImportOrder();
        ...
        private void cbStockUpdate_CheckedChanged(object sender, EventArgs e)
        {
            string msg;
            int cbVal = cbStockUpdate.Checked ? 1 : 0;
            bool result = _medicineService.SetSwitchInfo(0, cbVal, out msg);
            if(!result)
            {
                MessageBox.Show(msg);
            }
        ...
        private void cbPeriodUpdate_CheckedChanged(object sender, EventArgs e)
        {
            string msg;
            int cbVal = cbPeriodUpdate.Checked ? 1 : 0;
            bool result = _medicineService.SetSwitchInfo(1, cbVal, out msg);
            if (!result)
            {
                MessageBox.Show(msg);
            }
