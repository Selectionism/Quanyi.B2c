...
WHERE OrderNo IN ('{orderNoList}');");
        ...
        public void InsertBill(DataTable dt, StringBuilder sql, List<DbParameter> dbParameters, List<string> orderNoList)
        {
...
VALUES");
            ...
            {
                if(dt.Rows[i]["订单状态"]!=null && dt.Rows[i]["订单状态"].ToString().Contains("交易关闭"))
                {
                    continue;
                }
                if(!orderNoList.Exists(n=>n.Equals(dt.Rows[i]["订单编号"])))
                {
                    continue;
                }
        ...
        public StringBuilder GetProductInfo()
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"SELECT pro_id FROM product WHERE 1=1");
            //if (productList.Count > 0)
            //{
            //    string primaryKeys = string.Join(",", productList.Select(n => n.PrimaryKey).Distinct().ToArray()).Replace(",", "','");
            //    sql.AppendFormat(@"AND pro_id IN ('{0}');", primaryKeys);
            //}
            return sql;
        }

        public List<string> InsertBillPro(DataTable dt, StringBuilder sql, List<MedicineInfo> productList)
        {
...
VALUES");
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (dt.Rows[i]["商家编码"] != null && !string.IsNullOrEmpty(dt.Rows[i]["商家编码"].ToString()))
                {
                    //以+号进行分割，统计有多少个商品编码
                    string[] primaryKeyRows = dt.Rows[i]["商家编码"].ToString().Split('+');
                    //对分割而来的商品进行遍历，看看每个下商品又对应多少数量
                    for (int j = 0; j < primaryKeyRows.Length; j++)
                    {
                        string[] quantityRows = primaryKeyRows[j].Split('*');
                        if(!productList.Exists(n=>n.PrimaryKey.Equals(quantityRows[0])))
                        {
                            continue;
                        }
                        if (quantityRows.Length > 1)
                        {
                            sql.Append($@"('{dt.Rows[i]["订单编号"]}','{quantityRows[0]}',
                    '{Convert.ToInt32(dt.Rows[i]["购买数量"])* Convert.ToInt32(quantityRows[1])}','{dt.Rows[i]["价格"]}'),");
                        }
                        else
                        {
                            sql.Append($@"('{dt.Rows[i]["订单编号"]}','{quantityRows[0]}',
                    '{Convert.ToInt32(dt.Rows[i]["购买数量"])}','{dt.Rows[i]["价格"]}'),");
                        }
                        if(!orderNoList.Exists(n=>n.Equals(dt.Rows[i]["订单编号"])))
                        {
                            orderNoList.Add(dt.Rows[i]["订单编号"].ToString());
                        }
                    }
                }
            }
