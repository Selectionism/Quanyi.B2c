...
namespace Quanyi.B2c.Yaofangwang
{
    public class B2cPuller
    {
        ...
        private List<MedicineInfo> GetProductList()
        {
            using (var mysql = new MysqlHelper())
            {
                var sql = _billInfoSql.GetProductInfo();
                var dt = mysql.FillTable(sql.ToString());
                var proIds = dt.AsEnumerable().Select(c => c.Field<string>("pro_id")).ToList();
                List<MedicineInfo> productList = new List<MedicineInfo>();
                foreach (string pro_id in proIds)
                {
                    if(!productList.Exists(n=>n.PrimaryKey.Equals(pro_id)))
                    {
                        productList.Add(new MedicineInfo()
                        {
                            PrimaryKey = pro_id
                        });
                    }
        ...
        public void UploadBillExcelData(DataTable dtOrder, DataTable dtOrderDetail)
        {
            ...
            List<MedicineInfo> productList = GetProductList();
            List<string> orderNoList = _billInfoSql.InsertBillPro(dtOrderDetail, sqlOrderDetail, productList);
            _billInfoSql.InsertBill(dtOrder, sqlOrder, paramOrderList, orderNoList);
            //成功则记录数据库
            using (var mysql = new MysqlHelper())
            {
                //开启事务
                ...
                try
                {
                    ...
                    mysql.ExecuteNonQuery(sqlOrderDetail.ToString());
